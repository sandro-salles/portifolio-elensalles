...
<?php
	...
	wp_enqueue_script("jquery.ui", get_stylesheet_directory_uri()."/js/jquery.ui.js", false, THEMEVERSION, true);
	...
	{	
		...
		wp_enqueue_script("combined_js", get_stylesheet_directory_uri()."/cache/combined.js", false, THEMEVERSION, true);
	}
	else
	{
		foreach($js_arr as $file) {
			if($file != 'jquery.js' && $file != 'jquery-ui.js')
			{
				wp_enqueue_script($file, get_stylesheet_directory_uri()."/js/".$file, false, THEMEVERSION, true);
			}
		}
	}
?> 
...
<body <?php body_class(); ?> <?php if(is_home() && $pp_homepage_style == 'flow') { ?>data-gallery="flow"<?php } ?>>
	...
	<div class="top_bar fade-in one">
		...
		<div id="menu_wrapper">
			...
			<img id="mobile_menu" src="<?php echo get_stylesheet_directory_uri(); ?>/images/mobile_menu.png" alt=""/>
